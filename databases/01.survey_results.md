...
- Current applications landscape, development skills, project expectations are strongly based on relational models and the Serverless Relational Databases available does not provide a good alternative for non Serverless ones.
- Move to non Relational Databases (NoSQL) requires development of new skills and to adopt new constraints (no transactional guarantees, eventual consistency).
- Stakeholders are concerned about Data Governance and Security.
- There is not easy way to compare Serverless Databases with existent alternatives in terms of features and costs.

According to the answers, it is clear that the respondents recognize that new ways of using databases exists.  
And when Serverless Databases are mentioned, they are exclusively focused on this "new" ways (NoSQL, Managed Services, etc.).

But the answers indicates that the main part of the applications being developed today are still following the relational model.  
And for these applications and projects, move to a Serverless Databases open questions that are not easy to be addressed.

Here we understood that the challenges are not specific to the people that are answering the survey.  
But they are more broad, and involve other developers, project stakeholders, security departments and others.  
That is something that need to be addresses in a follow-up survey.
...
We will publish a new invitation in the next weeks about how can you join our effort and guide the discussions.
